...
func (s *Store) MultiGet(keysIn [][]byte) ([][]byte, error) {
	...
	C.rocksdb_readoptions_destroy(options)
	...
	for i, val := range vals {
		...
		if val != nil {
			valsOut[i] = append([]byte(nil), charToByte(val, val_lens[i])...)
		}
	}
